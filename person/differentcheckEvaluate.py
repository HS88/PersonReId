...
#	different_check_evaluation_xxx.txt : contain the final evaluation result (Map/Rank1 scores)
# 	different_pair_evaluation_xxx.txt  : Contain the query-test image and their similarity score
...
two = "_16"
three = "_100"
fn = "different_check_evaluation" + one + two + three + ".txt"  # file containing the output
pair = "different_pair_evaluation" + one + two + three + ".txt" #file containing the images that were considered similar along with their score
file = open(fn,"w")
pairfile = open(pair,"w")

# defining the tensor that will be used to get evaluation scores
...
# Model for which evaluation is to be done
trained_model = 'model_depth16_card16_width16_epoch125_.ckpt'
...
#we will put the test images in junk that are from same camera
#It means we will calculate the mAP score and Rank1 score considering
#images of same persons from different cameras only
...
# Tensor will be evaluated here to compute the feature values of images  
